...
//  ,`iptv1`.store_id as storeAttr,iptvi2`.store_id as storeAttr2,iptvi5`.store_id as storeAttr5,iptvi4`.store_id as storeAttr4,iptvi6`.store_id as storeAttr6,
            ...
            $tabForm->getSelect()->from(
                new \Zend_Db_Expr
                ("(SELECT `main_table`.*,
                iptv1.product_id as prodAttr,iptvi2.product_id as prodAttr2,iptvi5.product_id as prodAttr5,iptvi4.product_id as prodAttr4,iptvi6.product_id as prodAttr6,
                `iptv1`.`value` AS `label`, `iptvi2`.`value` AS `is_active`, `iptvi4`.`value` AS `content`, `iptvi3`.`value` AS `order`, `iptvi5`.`value` AS `show_purchased`, `iptvi6`.`value` AS `group`,cg.customer_group_code AS `groupname` FROM {$resource->getTableName('itoris_producttabs_tabs')} AS `main_table`
                 ...
                 WHERE ((iptv1.product_id IS NULL OR iptv1.product_id={$prod_id}) AND  (iptvi2.product_id IS NULL  OR iptvi2.product_id={$prod_id}) AND (iptvi5.product_id IS NULL OR iptvi5.product_id={$prod_id}) AND (iptvi3.product_id IS NULL OR iptvi3.product_id={$prod_id}) AND (iptvi4.product_id IS NULL OR iptvi4.product_id={$prod_id}) AND  (iptvi6.product_id IS NULL OR iptvi6.product_id={$prod_id})
                 AND iptv1.store_id IS NULL AND iptvi2.store_id IS NULL AND iptvi5.store_id IS NULL AND iptvi3.store_id IS NULL AND iptvi4.store_id IS NULL AND iptvi6.store_id IS NULL) AND main_table.tab_id={$tab_id} ORDER BY iptv1.product_id DESC ,iptvi2.product_id DESC,iptvi3.product_id DESC,iptvi4.product_id DESC,iptvi5.product_id DESC,iptvi6.product_id DESC )
          "))->limit(1);
            ...
            $tabForm->getSelect()->from(
                new \Zend_Db_Expr
                ("(SELECT `main_table`.*,  iptv1.product_id as prodAttr,iptvi2.product_id as prodAttr2,iptvi5.product_id as prodAttr5,iptvi4.product_id as prodAttr4,iptvi6.product_id as prodAttr6,
                `iptv1`.store_id as storeAttr,iptvi2.store_id as storeAttr2,iptvi5.store_id as storeAttr5,iptvi4.store_id as storeAttr4,iptvi6.store_id as storeAttr6,
                `iptv1`.`value` AS `label`, `iptvi2`.`value` AS `is_active`, `iptvi4`.`value` AS `content`, `iptvi3`.`value` AS `order`, `iptvi5`.`value` AS `show_purchased`, `iptvi6`.`value` AS `group`,cg.customer_group_code AS `groupname` FROM {$resource->getTableName('itoris_producttabs_tabs')} AS `main_table`
                 INNER JOIN `{$resource->getTableName('itoris_product_tabs_value_varchar')}` AS `iptv1` ON main_table.tab_id = iptv1.tab_id AND iptv1.attribute_id=1
                 INNER JOIN `{$resource->getTableName('itoris_product_tabs_value_int')}` AS `iptvi2` ON main_table.tab_id = iptvi2.tab_id AND iptvi2.attribute_id=2

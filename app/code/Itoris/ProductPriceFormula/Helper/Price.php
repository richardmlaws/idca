...
class Price extends Data
{
    ...
    public function getProductFinalPrice($item) {
        ...
        $options = $this->_getBuyRequest($item)->getOptions();
        $optionsQty = (array) $this->_getBuyRequest($item)->getOptionsQty();
        ...
        foreach ($conditionPrice as $formulaId => $values) {
            ...
            foreach ($values as $value) {
                if (!$isRightCondition) {
                    ...
                    if ($condition != '') {
                        $formulaFunc = create_function('&$isRightCondition, &$priceForCompare', 'if (' . $condition . ') {$isRightCondition=true; $priceForCompare = round(' . $priceCond . ', 2);}');

...
Itoris.PriceFormula = {
    initialize : function(conditions, optionsData, specialPrice, dataBySku, productId, conversionRate) {
		if (!jQuery.mage || !jQuery.mage.priceOptions) {
			var _this = this;
			//wait until priceOptions widget is loaded and initialized
			setTimeout(function(){_this.initialize(conditions, optionsData, specialPrice, dataBySku, productId, conversionRate);}, 200);
			return;
		}
		...
		this.getPriceBox().on('reloadPrice', function(ev){
            ...
            if (!basePriceObj[0]) basePriceObj = jQuery('.product-info-main [data-price-type="basePrice"] .price');
			var finalPriceObj = jQuery('#itoris_dynamicproductoptions_popup_price [data-price-type="finalPrice"] .price');
			if (!finalPriceObj[0]) finalPriceObj = jQuery('.product-info-main [data-price-type="finalPrice"] .price');
			...
			var price = (baseInitialAmount ? basePriceObj[0] : finalPriceObj[0]).innerHTML.replace(/[^0-9]+/g,"")/100;
			...
			if (priceForCompare > 0) {
				finalPriceObj.text(window._priceUtils.formatPrice(curObj.finalPrice * taxRate));
				if (basePriceObj[0] && basePriceObj[0] !== finalPriceObj[0]) basePriceObj.text(window._priceUtils.formatPrice(curObj.finalPrice));
			}

...
window.DynamicProductOptions = {
    ...
    initialize : function(config, options, isGrouped) {
        if ((!jQuery.mage || !jQuery.mage.priceOptions && !this.getPriceBox()[0]) && options.length) {
            ...
            setTimeout(function(){obj.initialize(config, options, isGrouped);}, 200);
            return;
        }
    ...
    prepareOptions : function() {
        ...
        this.getPriceBox().on('updatePrice', function(ev, prices){
            if (!prices) return;
			var basePriceObj = jQuery('.product-info-main [data-price-type="basePrice"] .price');
			...
			var taxRate = (baseInitialAmount && finalInitialAmount) ? finalInitialAmount / baseInitialAmount : 1;
            ...
            for(key in prices) {
                ...
                if (qtyElm && prices[key].basePrice) {
                    ...
                    prices[key].finalPrice.amount = prices[key].dpoPrice.amount * taxRate * qty;
                    
                    var tierPrice = obj.getTierPrice(qtyElm);
                    if (tierPrice) {
                        prices[key].basePrice.amount = tierPrice * qty;
                        prices[key].finalPrice.amount = tierPrice * taxRate * qty;
                    }
                }

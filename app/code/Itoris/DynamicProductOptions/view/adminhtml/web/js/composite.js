...
define([
    ...
    DynamicProductOptions = Class.create();
    DynamicProductOptions.prototype = {
        dateFormat : 'm/d/y',
        groupProducts: [],
        ...
        initialize : function(config, options, isGrouped) {
            ...
            this.productForm = $('product_composite_configure_form');
        ...
        initForm : function() {
            if (this.config.is_grouped) {
                ...
                this.addToCartButton = $$('.modal-header .page-actions-buttons .action-primary')[0];
                if (!this.addToCartButton) {
                    return;
                }

                var optionsWrapper = $('product_composite_configure_fields_options');
                ...
                if (this.getAppearance() == 'popup_cart' || this.getAppearance() == 'popup_configure') {
                    ...
                    if (form) {
                        ...
                        if (optionBottomBlock) {
                            ...
                            var addToCart = optionBottomBlock.select('#product-addtocart-button')[0];
							if (!addToCart) var addToCart = optionBottomBlock.select('#product-updatecart-button')[0];
                    ...
                    if (optionsWrapper) {
                        optionsWrapper.select('.swatch-opt, .configurable, #itoris_dynamicproductoptions').each(function(swatch){
        ...
        getAppearance : function() {
            return 'on_product_view';//this.config.appearance;
        },
        ...
        prepareOptions : function() {
			...
			this.getPriceBox().on('updatePrice', function(ev, prices){
				...
				for(key in prices) {
					var _key = key;
					var qtyElm = obj.updateQtyElement(key); //alert(JSON.stringify(prices));
                    if (qtyElm && prices[key].finalPrice) {
                        var qty = parseInt(qtyElm.value);
                        if (isNaN(qty) || qty < 1) qty = 1;
						if (!prices[key].dpoPrice) prices[key].dpoPrice = {amount: prices[key].finalPrice.amount};
						prices[key].finalPrice.amount = prices[key].dpoPrice.amount * qty;
						
						var tierPrice = obj.getTierPrice(qtyElm);
						if (tierPrice) prices[key].finalPrice.amount = tierPrice * qty;
                    }
				}
        ...
        updateQtyElement : function(key) {
            ...
            elms.each(function(elm){
                if (elm.tagName == 'INPUT') {
                    if (typeof elm.use_qty != 'undefined' && elm.use_qty) {
                ...
                } else if (typeof elm.qty_values != 'undefined' && elm.qty_values.length) {
                    var qtyElm = obj.addQtyElement(elm);
                    if (elm.qty_values.indexOf(elm.value) == -1 && qtyElm) {
                        qtyElm.up('div').remove();
                    } else {
                        obj.updateQtyComment(qtyElm);
                        _qtyElm = qtyElm;
        ...
        updateQtyComment: function(qtyElm) {
            ...
            if (tierPrices.length > 0) {
                ...
                for(var i=0; i<tierPrices.length; i++) {
                    tierList += '<div>Buy <b>'+tierPrices[i].qty+'</b> for <b>'+ formatCurrency(parseFloat(tierPrices[i].price), this.getPriceBox().data('magePriceBox').options.priceConfig.priceFormat, false) +'</b> each</div>';
                }
        ...
        getTierPrice: function(qtyElm) {
            var qty = parseInt(qtyElm.value), price = '', tierPrices = this.getTierPrices(qtyElm);
            if (tierPrices.length > 0) {
        ...
        addQtyElement: function(elm) {
            ...
            if ($(qtyId)) {
                ...
                elmQty._parent = elm;
                elmQty.observe('change', function(){
                    jQuery(elm).trigger('change');
                }.bind(this));
        ...
        hideField: function(field) {
            ...
            if (field.tagName.toLowerCase() == 'option') {
                ...
                if (select) select.removeChild(field);
            }
        ...
        showField: function(field) {
            ...
            if (field.tagName.toLowerCase() == 'option' && !field.up() && field._parent) {
                ...
                field._parent.select('option').each(function(option){option.up().removeChild(option)});
                    ...
                    if (typeof this.options[i].items != 'undefined') {
                        ...
                        for (var j = 0; j < this.options[i].items.length; j++) {
                            ...
                            if(elm !== undefined && this.options[i].items[j] !== undefined && this.options[i].items[j].elm !== undefined && this.options[i].items[j].elm._parent !== undefined) {
                                if (elm && this.options[i].items[j].elm._parent && (this.options[i].items[j].elm._parent.up('div.field').hasClassName('ihidden')
                                    || this.options[i].items[j].elm._parent.up('div.fieldset').hasClassName('ihidden')) ||
                                    elm && !this.options[i].items[j].elm._parent && (this.options[i].items[j].elm.up('div.field').hasClassName('ihidden')
                                    || this.options[i].items[j].elm.up('div.fieldset').hasClassName('ihidden'))
                                ) {
        ...
        isConditionCorrect : function(condition) {
            ...
            if (condition.type == 'field') {
                return this.isCorrect(condition.value, condition.condition, this.getOptionValue(this.getOptionByInternalId(condition.field), false, true))
            } else {
        ...
        showPopup : function() {
			...
			jQuery(this.popup).animate(beforeStyle, 400, 'swing', function(effect) {$('itoris_dynamicproductoptions_popup').setStyle({height:'auto'});});
            if (!this.config.is_grouped) {
			...
			$('itoris_dynamicproductoptions_popup_price').insert($$('[data-role=priceBox]')[0]);
        },
        ...
        hidePopup : function() {
			...
			$$('.product-info-main .product-info-price')[0].insert({top: $$('[data-role=priceBox]')[0]});
        },
        ...
        getOptionValue : function(option, withImages, asArray) {
                ...
                case 'multiple':
                    ...
                    for (var i = 0; i < options.length; i++) {
                        if (options[i].value && options[i].selected) {
                            value.push(options[i].origTitle);
                        }
                    }
                    if (asArray) {
                        return value;
		...
		getPriceBox: function(){
			return jQuery('[data-role=priceBox]');
		},

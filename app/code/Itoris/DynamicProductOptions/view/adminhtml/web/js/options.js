...
define([
    ...
    Itoris.OptionManager.prototype = {
        ...
        initialize : function(config, sections) {
			...
			jQuery('[data-form=edit-product]').on('submit', function(){_this.prepareDataBeforeSubmit();});
        ...
        toggleUseGlobal: function(){
            ...
            $('options_templates_div').style.display = this.config.store_id > 0 ? 'none' : 'block';
                ...
                if (tempSections[i]) {
                    for (var j = 0; j < tempSections[i].fields.length; j++) {
                        if (tempSections[i].fields[j]) {
                            ...
                            tempSections[i].fields[j]['option_id'] = 0;
        ...
        updateFieldConfigFields : function(field) {
            ...
            if (field.items) {
                var itemsParams = [
                    'title', 'price', 'price_type', 'sku', 'image_src', 'use_qty', 'is_selected', 'is_disabled', 'carriage_return','sort_order',
        ...
        addProductLinkToItem : function(productId) {
            var skuElm = this.productGridItemRow.select('input.dynamicoptions-value-sku')[0];
            skuElm.value = productId;
            this.productGridItemConfig.sku = productId;
        ...
        saveTierPrices: function(button) {
            ...
            tbody.select('tr').each(function(tr){
                ...
                if (tier.qty > 1 && !isNaN(tier.price)) tierPrice[tierPrice.length] = tier;
            });
    ...
    Itoris.DynamicProductOptionsTemplate.prototype = {
        ...
        loadTemplate : function() {
            if (parseInt(this.loadTemplatesDropdown.value)) {
                if (confirm('Do you really want to load this template? Current configuration will be lost!')) {

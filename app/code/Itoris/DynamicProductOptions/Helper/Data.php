...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function applyToProduct($productId, $objectToStore) {
        ...
        foreach($objectToStore as $storeId => &$object) {
            ...
            $sections = $object->setStoreId($storeId)->getSections();
                    ...
                    foreach ($section['fields'] as $index => &$field) {
                        if ($field) {
                            ...
                            if (!in_array($field['type'], ['image', 'html'])) {
                                if ($field['internal_id'] && $field['internal_id'] > 0 && isset($mapOptions[$field['internal_id']])) {
                                    ...
                                    $con->insert($res->getTableName('catalog_product_option'), [
                                        'option_id'         => $this->getMaxDb('catalog_product_option', 'option_id'),
                                    ...
                                    $optionId = intval($con->fetchOne("select max(`option_id`) from {$res->getTableName('catalog_product_option')}"));
                            ...
                            if (isset($field['items'])) {
                                ...
                                if ($storeId > 0) {
                                    $_values = [];
                                    foreach($defaultStoreFields[(int) $field['internal_id']]['items'] as $value) if ($value) {
                                        if (!isset($value['option_type_id'])) $value['option_type_id'] = 'tmp'.$optionId.'_'.$value['order'];
                                        $_values[$value['option_type_id']] = $value;
                                    }
                                    foreach ($field['items'] as $index => $value) if ($value) {
                                        //checkig if value was removed in the default config, we should remove it in the store view config too
                                        if (!isset($value['option_type_id'])) $value['option_type_id'] = 'tmp'.$optionId.'_'.$value['order'];
                                        if (!isset($_values[$value['option_type_id']])) {
                                            unset($field['items'][$index]);
                                            continue;
                                        }
                                        
                                        //track new values in the default config, we should add them to the end of the store view config
                                        unset($_values[$value['option_type_id']]);
                                    }                                    
                                    ...
                                    if ($value) {
                                        ...
                                        if (isset($mapValues[$value['option_type_id']])) {
                                            ...
                                            $con->insert($res->getTableName('catalog_product_option_type_value'), [
                                                'option_type_id'         => $this->getMaxDb('catalog_product_option_type_value', 'option_type_id'),
                                                'option_id'         => $optionId,
                                                'sku'                 => isset($value['sku']) ? $value['sku'] : NULL,
                                                'sort_order'         => isset($value['sort_order']) ? $value['sort_order'] : 0
                                            ]);
                                            $valueId = intval($con->fetchOne("select max(`option_type_id`) from {$res->getTableName('catalog_product_option_type_value')}"));
            ...
            $con->insert($res->getTableName('itoris_dynamicproductoptions_options'), [
                'config_id'         => $this->getMaxDb('itoris_dynamicproductoptions_options', 'config_id'),
                'product_id'         => $productId,
                'store_id'             => $storeId,
    ...
    public function getMaxDb($table, $column){
        $res = $this->_objectManager->get('Magento\Framework\App\ResourceConnection');
        $con = $res->getConnection('write');

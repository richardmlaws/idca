...
class Select extends \Magento\Catalog\Model\Product\Option\Type\Select
{
    ...
    protected function _getValueImageSrc($valueId) {
        ...
        if ($img_src) {
            if (!$this->_httpProtocol) {
                ...
                $baseUrl = $is_frontend_use_secure ? $this->_backendConfig->getValue('web/secure/base_url') : $this->_backendConfig->getValue('web/unsecure/base_url');
    ...
    public function getOptionPrice($optionValue, $basePrice) {
        ...
        if ($canUseQtys) {
            $option = $this->getOption();
            $result = 0;
            $optionQty = $optionQtys[$option->getId()];
            if (!$this->_isSingleSelection()) {
                foreach(explode(',', $optionValue) as $value) {
                    if ($_result = $option->getValueById($value)) {
                        $qty = isset($optionQty[$value]) ? (int)$optionQty[$value] : 1;
                        list($price, $price_type) = $this->getTierPriceByQty($value, $qty, $_result->getPrice(), $_result->getPriceType());
                ...
                if ($_result = $option->getValueById($optionValue)) {
                    list($price, $price_type) = $this->getTierPriceByQty($optionValue, (int)$optionQty, $_result->getPrice(), $_result->getPriceType());
                    $result = $this->_getChargableOptionPrice(
                        $price * (int)$optionQty,
                        $price_type == 'percent',
                        $basePrice

...
EffectDesigner.prototype = {
    initialize: function() {
        ...
        var filters = ['grayscale', 'invert', 'remove-white', 'sepia', 'sepia2', 'brightness', 'noise', 'gradient-transparency', 'pixelate', 'blur', 'sharpen', 'emboss', 'tint', 'multiply', 'blend'];
        var filter = {};
    },
...
ProductDesigner.prototype = {
    ...
    initialize: function(data, loginUrl, logindesignUrl, registerUrl, downloadUrl, saveDesignUrl, continueUrl, productUrl, allowDownload, brushUrl, previewImageUrl, addtocarturl, getmydesignUrl, facebookUrl, googleUrl, sendmailUrl, isAdmin, productdesignerUrl) {
        if (data.action == 'full') {
            ...
            if (obj && (obj.type == 'text' || obj.type == 'group')) {
                if (obj.type == 'group') {
                    ...
                    if (text != oldText) {
                        ...
                        var defaultArc = obj.arc ? obj.arc : 0;
                        var defaultSpacing = obj.spacing ? obj.spacing : 0;
                        cmd = new TextSpaceAngleChange(this.productDesigner, canvas, { arc: defaultArc, spacing: defaultSpacing }, { arc: currentArc, spacing: currentSpacing });
                        cmd.exec();
                        History.prototype.push(cmd);

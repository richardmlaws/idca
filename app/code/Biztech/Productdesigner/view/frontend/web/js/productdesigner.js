...
GroupDesigner.prototype = {
    groupSide: null,
    sideChanged: null,
    defaultTextOpt: {}, initialize: function (textSize, textFamily, sizeArray) {
        ...
        for (var i = 0; i < allObj.length; i++)
        {
            if (allObj[i].name == 'groupTabNumber')
            {
                ...
                if (allObj[i].text != jQuery(index).val() && jQuery(index).val() != '') {
                    ...
                    var text = jQuery(index).val();
                    var textObjectData = {fontSize: allObj[i].fontSize,
                        fontFamily: jQuery('#group_font_selection').val(),
                        fill: '#000000',
                        top: allObj[i].top,
                        left: allObj[i].left,
            ...
            for (var imageId in this.productDesigner.containerCanvases) {
                ...
                if (imageId == GroupDesigner.prototype.groupSide) {
                    ...
                    for (var i = 0; i < objects.length; i++) {
                        ...
                        objects[i].set({change_side_group: true,
                        });
                ...
                success: function (data, textStatus, jqXHR) {
                    ...
                    if (response.design_id != null && response.design_id != '') {
                        ...
                        if (jQuery('#product-sides')[0].children[1].children[0] != undefined) {
                            jQuery('#product-sides')[0].children[1].children[0].click();
                        }
...
//                                jQuery('#product-sides')[0].children[1].children[1].children[0].click();
//                            }
//                            if (jQuery('#product-sides')[0].children[1].children[0] != undefined) {
...
MyDesigns.prototype = {filter_design_url: null,
    delete_design_url: null,
    ImageSideObject: {},
    initialize: function (filter_design_url, delete_design_url, data1)
    {
...
ProductDesigner.prototype = {
    opt: {product_side_id: 'product-sides'
    },
    canvas: null,
    data: null,
    pricesContainers: {},
    ...
    addtocarturl: null,
    mediaUrl: null,
    initialize: function (data, loginUrl, logindesignUrl, registerUrl, downloadUrl, saveDesignUrl, continueUrl, productUrl, allowDownload, brushUrl, previewImageUrl, addtocarturl, getmydesignUrl, facebookUrl, googleUrl, sendmailUrl, isAdmin, mediaUrl) {
    ...
    observeObjectModified: function () {
        ...
        this.canvas.observe('object:modified', function (e) {
            ...
            if (currentState.angle != target.angle) {
                ...
                var obj = this.canvas.getActiveObject();
                var cmd = new RotateObjectHistory(this.canvas, obj, {angle: currentState.angle, left: currentState.left, top: currentState.top}, {angle: target.angle, left: e.target.left, top: e.target.top});
                History.prototype.push(cmd);
            }
            ...
            if (currentState.scaleX != target.scaleX || currentState.scaleY != target.scaleY) {
                var cmd = new ResizeObjectHistory(
                        this.canvas,
                        this.canvas.getActiveObject(),
                        {scaleX: currentState.scaleX, scaleY: currentState.scaleY},
    ...
    observeCanvasObjectRendered: function () {
        ...
        this.canvas.observe('after:render', function (e) {
            ...
            /*if (n > 0) {
             ...
             $('print_area_warning').show();
             } else {
             this.designArea.style.border = '1px dashed';
             $('print_area_warning').hide();
             }*/
...
//            this.removeAllSelectedObject(e.target.canvas);
////            //jQuery("#advanceTools").removeAttr("disabled");
//            ProductDesigner.prototype.canvas = e.target.canvas;
//            this.objSelectEvent(e.target.canvas.getActiveObject());
    ...
    observerClipartColorChange: function () {
        jQuery('#clipart-color-container').on('click', '.clipart-color-img', function (event) {
            ...
            this.productDesigner = window.ProductDesigner;
            var canvas = ProductDesigner.prototype.canvas;
            var obj = canvas.getActiveObject();
    ...
    reloadPrice: function () {
        ...
        if (OptionsPrice != undefined) {
            ...
            OptionsPrice.addCustomPrices('design_custom_image_price', {price: customImagesPrice, type: 'fixed'});
    ...
    observeControls: function () {
        ...
        jQuery('#obj_flip_horizantal').on('click', function (e) {
            ...
            var cmd = new UpdateCommand(canvas, obj, {flipX: flip});
            cmd.exec();
            History.prototype.push(cmd);
        }.bind(this));
        /**
        ...
        jQuery('#obj_flip_vertical').on('click', function (e) {
            ...
            var cmd = new UpdateCommand(canvas, obj, {flipY: flip});
            cmd.exec();
            History.prototype.push(cmd);
        }.bind(this));
        /**
            ...
            if (!ProductDesigner.prototype.canvasesHasDesigns()) {
                ...
                function func2()
                {
                    ...
                    if (i == 100) {
                        ...
                        jQuery('.progress-bar-wrapper').hide();
                    }
                }
...
//                    {
                    ...
                    if (canvas.masking_data != undefined)
                    {
                        fabric.loadSVGFromURL(canvas.masking_data, function (objects, options) {
                            var cmd = new ObjectMaskingHistory(canvas, objects, options, canvas.masking_data, null);
                            cmd.exec();
                            History.prototype.push(cmd);
                        });
                    }
                    ...
                    ProductDesigner.prototype.productImageSize = {
                        height: productNew.height * (1 / ProductDesigner.prototype.scaleFactor), width: productNew.width * (1 / ProductDesigner.prototype.scaleFactor)
                    }
    ...
    reloadPrintingPrice: function () {
        ...
        this.designerPrices['custom_image_price'] = customImagesPrice;
        if (OptionsPrice != undefined) {
            OptionsPrice.addCustomPrices('fixed_price', {price: fixedPrice, type: 'fixed'});
            OptionsPrice.addCustomPrices('design_text_price', {price: textsPrice, type: 'fixed'});
            OptionsPrice.addCustomPrices('design_image_price', {price: imagesPrice, type: 'fixed'});
            OptionsPrice.addCustomPrices('design_custom_image_price', {price: customImagesPrice, type: 'fixed'});
            OptionsPrice.setDuplicateIdSuffix('-design');
            ...
            if (ProductDesigner.prototype.data.productType == "configurable") {
                ...
                if (document.querySelector('input[name="printing_type"]:checked').getAttribute('data-code') == 'silk-screen') {
                    ...
                    if (ProductDesigner.prototype.data.product.printing_method_array[selected_printing_method]['printing_type'] == 1) {
                        ...
                        var tier_price_details = ProductDesigner.data.data.product.printing_method_array[selected_printing_method]['product_details'];
                            ...
                            if (str[1] == 'small') {
                                ...
                                for (var j = 0; j < side_count.length; j++) {
                                    ...
                                    if (area_by_side[str[1]] == side_count[j]) {
                                        ...
                                        if (side_count[j] == 'front') {
                                            var prod_side = str[1] + '_' + str[0];
                                        }
                                        if (side_count[j] == 'back') {
                                            var prod_side = str[1] + '_' + str[0];
                                        }

                                        if (side_count[j] == 'left') {
                                            var prod_side = str[1] + '_' + str[0];
                                        }

                                        if (side_count[j] == 'right') {
                                            var prod_side = str[1] + '_' + str[0];
                                        }
    ...
    return {
        exec: function () {
            ...
            if (obj.type == 'text' || obj.type == 'group') {
                ...
                layer_img.setAttribute('class', 'sprite ico-layer-text');
                layer_span_layer.appendChild(layer_img);
            } else if ((obj.type == 'path') || (obj.type == 'group' && obj.tab != 'textTab')) {
...
QuotesDesigner.prototype = {initialize: function (filter_url) {
...
TextDesigner.prototype = {
    ...
    observeTextFontChange: function () {
        ...
        jQuery('.font_selection').on('selectric-change', function (element) {
            ...
            if (obj && (obj.type == 'text' || obj.type == 'custom_text' || obj.type == 'group')) {
                ...
                var cmd = new UpdateCommand(canvas, obj, {fontFamily: jQuery('#font_selection').val()});
                if (obj.tab == 'grouporder') {
                    var cmd = new UpdateCommand(canvas, obj, {fontFamily: jQuery('#group_font_selection').val()});
                } else {
                    if (obj.text_type == 'quote')
                        var cmd = new UpdateCommand(canvas, obj, {fontFamily: jQuery('#font_selection').val()}, "center");
                    else
                        var cmd = new UpdateCommand(canvas, obj, {fontFamily: jQuery('#font_selection').val()});
                }
                cmd.exec();
    ...
    setTextColor: function (color) {
        ...
        var obj = canvas.getActiveObject();
        if (obj && (obj.type == 'text' || obj.type == 'group')) {
            var cmd = new UpdateCommand(canvas, obj, {fill: color});
            cmd.exec();
    ...
    setTextBgColor: function (color) {
        ...
        if (obj && (obj.type == 'text' || obj.type == 'group')) {
            if (!jQuery('#text_bg_null').checked) {
                var cmd = new UpdateCommand(canvas, obj, {textBackgroundColor: obj.textBackgroundColor != color ? color : ''});
                cmd.exec();
    ...
    setTextStrokeColor: function (color) {
        ...
        if (obj && (obj.type == 'text' || obj.type == 'group')) {
            if (!jQuery('#text_stroke_color').checked) {
                ...
                var cmd = new UpdateCommand(canvas, obj, {stroke: stroke_color});
                cmd.exec();
    ...
    setTextShadowColor: function (color) {
        ...
        if (obj && (obj.type == 'text' || obj.type == 'group')) {
            ...
            var cmd = new UpdateCommand(canvas, obj, {shadow: shadow});
            cmd.exec();
    ...
    observeTextSizeChange: function () {
        ...
        jQuery('#font_size_selection').on('change input', function () {
            ...
            var obj = canvas.getActiveObject();
            if (obj && (obj.type == 'text' || obj.type == 'group')) {
                var cmd = new UpdateCommand(canvas, obj, {fontSize: jQuery('#font_size_selection').val()});
                cmd.exec();
                History.prototype.push(cmd);
            }
    ...
    observeTextStyleChange: function () {
        ...
        jQuery('#add_btn_bold').on('click', function (e) {
            ...
            if (obj && (obj.type == 'text' || obj.type == 'group')) {
                ...
                var cmd = new UpdateCommand(canvas, obj, {fontWeight: fontWeight});
                cmd.exec();
                History.prototype.push(cmd);
        ...
        jQuery('#add_btn_italic').on('click', function (e) {
            ...
            if (obj && (obj.type == 'text' || obj.type == 'group')) {
                ...
                var cmd = new UpdateCommand(canvas, obj, {fontStyle: fontStyle});
                cmd.exec();
                History.prototype.push(cmd);
        ...
        jQuery('#add_btn_underline').on('click', function (e) {
            ...
            if (obj && (obj.type == 'text' || obj.type == 'group')) {
                ...
                var cmd = new UpdateCommand(canvas, obj, {textDecoration: textDecoration});
                cmd.exec();
                History.prototype.push(cmd);
        ...
        jQuery('#reset_shadow').on('click', function (e) {
            ...
            if (obj && (obj.type == 'text' || obj.type == 'group')) {
                var cmd = new UpdateCommand(canvas, obj, {shadow: ''});
                cmd.exec();
                History.prototype.push(cmd);
        ...
        jQuery('#opacity').on('change input', function () {
            ...
            if (obj && (obj.type == 'text' || obj.type == 'group')) {
                ...
                if (obj.opacity != text_opacity) {
                    var cmd = new UpdateCommand(canvas, obj, {opacity: text_opacity});
                    cmd.exec();
                }
                History.prototype.push(cmd);
            }
        ...
        jQuery('#btn_left_align').on('click', function (e) {
            ...
            if (obj && (obj.type == 'text' || obj.type == 'custom_text')) {
                var cmd = new UpdateCommand(canvas, obj, {textAlign: 'left'});
                cmd.exec();
                History.prototype.push(cmd);
        ...
        jQuery('#btn_right_align').on('click', function (e) {
            ...
            var obj = this.productDesigner.canvas.getActiveObject();
            if (obj && (obj.type == 'text' || obj.type == 'custom_text')) {
                var cmd = new UpdateCommand(canvas, obj, {textAlign: 'right'});
                cmd.exec();
                History.prototype.push(cmd);
        ...
        jQuery('#btn_center_align').on('click', function (e) {
            ...
            var obj = this.productDesigner.canvas.getActiveObject();
            if (obj && (obj.type == 'text' || obj.type == 'custom_text')) {
                var cmd = new UpdateCommand(canvas, obj, {textAlign: 'center'});
                cmd.exec();
                History.prototype.push(cmd);
        ...
        jQuery('#reset_stroke').on('click', function (e) {
            ...
            if (obj && (obj.type == 'text' || obj.type == 'group')) {
                var cmd = new UpdateCommand(canvas, obj, {stroke: '', strokeWidth: '1'});
                cmd.exec();
                History.prototype.push(cmd);
                jQuery('#btn_font_effect').each(function (index, val) {
                    jQuery(index.id).removeClass('active');
        ...
        jQuery('#stroke_width').on('change input', function () {
            ...
            if (obj && (obj.type == 'text' || obj.type == 'group')) {
                ...
                if (obj.strokeWidth != stroke_width) {
                    var cmd = new UpdateCommand(canvas, obj, {stroke: stroke_color, strokeWidth: stroke_width});
                    cmd.exec();
                }
                History.prototype.push(cmd);
            }
        ...
        jQuery('#add_btn_stroke').on('click', function (e) {
            ...
            if (obj && (obj.type == 'text' || obj.type == 'group')) {
                ...
                if (obj.strokeWidth == '1' && obj.type != 'group') {
                    var cmd = new UpdateCommand(canvas, obj, {stroke: stroke_color, strokeWidth: this.defaultTextOpt.strokeWidth});
        ...
        jQuery('#text_bg_null').on('click', function (e) {
            ...
            if (obj && (obj.type == 'text' || obj.type == 'group')) {
                ...
                var cmd = new UpdateCommand(canvas, obj, {textBackgroundColor: ''});
                cmd.exec();
                History.prototype.push(cmd);
...
var AlignToCenter = function (canvas, obj) {
    // save original state
    var prop = {left: obj.left, top: obj.top};
    return {
        exec: function () {

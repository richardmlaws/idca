...
define([
    ...
    $.widget('mage.productGallery', {
        ...
        _setImageSide: function (event, data) {
            this.element.find('.image_side').each($.proxy(function (index, element) {
    ...
    $.widget('mage.productGallery', $.mage.productGallery, {
        ...
        _initDialog: function () {
            ...
            $dialog.on('change', '[data-role=type-selector]', $.proxy(this._notifyType, this));
             $dialog.on('change', '#image-side', _changeImageSide);            
        ...
        onDialogOpen: function (event) {            
            ...
            $(event.target)
                .find('[data-role=type-imageSide]')
                .each($.proxy(function (index, checkbox) {

                    var isChecked = checkbox.value == imageData.image_side_default;
                            var targetName = checkbox.parentElement.getAttribute('name');
                            $('select[name="'+ targetName + '"] option[value='+checkbox.value+']').prop(
                                'selected',
                                isChecked
                            );
                        }, this));

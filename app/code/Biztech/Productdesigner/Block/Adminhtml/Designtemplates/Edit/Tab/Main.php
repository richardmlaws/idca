...
namespace Biztech\Productdesigner\Block\Adminhtml\Designtemplates\Edit\Tab;
...
class Main extends Generic implements TabInterface {
    ...
    protected function _prepareForm() {
        ...
        $fieldset = $form->addFieldset('base_fieldset', ['legend' => __('Edit Design Category')]);
        if ($model->getId()) {
        ...
        $eventElem = $fieldset->addField(
                'parent_categories', 'select', ['name' => 'parent_categories', 'label' => __('parent_categories'), 'title' => __('parent_categories'), 'disabled' => false, 'values' => $template_array]
        );
        $eventElem->setAfterElementHtml('<script>
                ...
                if(jQuery("#is_root_category").checked){
                //$("parent_categories").disabled=true;
                jQuery("#parent_categories").prop("disabled",true);
                }
                else{
                //$("parent_categories").disabled=false;

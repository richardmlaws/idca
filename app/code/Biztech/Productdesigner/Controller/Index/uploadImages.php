        ...
        try {
            ...
            if (isset($data['image_upload'])) {
                ...
                foreach ($data['image_upload'] as $key => $detail) {
                    ...
                    $max_size = $config->getValue('productdesigner/customimageuploadconfiguration/maximumsizeofimage');
                    if (isset($max_size) && $max_size != null) {
                        if($file_size / 1024 / 1024 > $max_size)
                        {
                        $result["error_message"] = __('You can not upload files larger than ' . $max_size . ' MB');
                        $result["status"] = 'fail';
                        $this->getResponse()->setBody(json_encode($result));
                        return false;
                        }

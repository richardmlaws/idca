   ...
   protected $image;
    ...
    public function execute() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if(isset($params['data']['design']))
            $template_id = $params['data']['design'];
            $images = $params['data']['images'];
            $large_images = $params['data']['large_images'];
            $saveDesign = $this->_saveDesign($images, $large_images,$template_id);
        }
     ...
     public function _saveDesign($images, $large_images,$template_id) {
        ...
        foreach ($layers as $key => $layer) {
            $ext = '';
            $img = isset($layer[0]->image_url) ? 1 : 0;
            if ($img) {
...
//               
        ...
        if ($template_id != null) {
            $designModel = $objectManager->create('Biztech\Productdesigner\Model\Designtemplates')->load($template_id);
            ...
            $designModel = $objectManager->create('Biztech\Productdesigner\Model\Designtemplates');
            $designModel->setProductId($prod_id)
                    ->setColorId($color_id)
                    ->setLayers($params['data']['layers'])
                    ->setLayerImages(json_encode($layer_images))
...
//
        ...
        if ($template_id != null) {
            $designImagesModelCol =  $objectManager->create('Biztech\Productdesigner\Model\Mysql4\Designtemplateimages\Collection')->addFieldToFilter('designtemplates_id', array('eq' => $template_id))->getData();
            foreach ($designImagesModelCol as $col) {
                $a = $objectManager->create('Biztech\Productdesigner\Model\Designtemplateimages')->load($col['image_id']);
                $a->delete();
            }
        }
        ...
        foreach ($merged_images as $key => $image) {
            foreach ($grouped_product_images as $product_image) {
                ...
                if ($product_image->getId() == $key) {
                    ...
                    $design_images = $this->saveDesignImages($prod_image_path, $image, $large_image, $image_id);
                    ...
                    try {
                        ...
                        if ($template_id != null) {
                            foreach ($design_images as $image_key => $design_image) {
                                ...
                                $designImagesModel->setDesigntemplatesId($template_id)                                        
                                        ->setProductImageId($newkey)
                            ...
                            foreach ($design_images as $image_key => $design_image) {
                                ...
                                $designImagesModel = $objectManager->create('Biztech\Productdesigner\Model\Designtemplateimages');
                                $designImagesModel->setDesigntemplatesId($designModel->getId())
                                        ->setDesignImageType($image_key)
...
//                        $obj_product = $objectManager->create('Magento\Catalog\Model\Product');
//                        $product = $obj_product->load($prod_id);
//                        $designImagesModel = $objectManager->create('Biztech\Productdesigner\Model\Resource\Designtemplateimages\Collection');
...
//                        $result['pinshare'] = "http://pinterest.com/pin/create/button/?url=" . urlencode($a . "productdesigner/?id=" . $prod_id . "&design=" . $designModel->getId()) . "&media=" . urlencode($media_path) . "&description=" . urlencode("My Custom Design");
    ...
    public function saveDesignImages($prod_image_path, $image, $large_image, $image_id) {

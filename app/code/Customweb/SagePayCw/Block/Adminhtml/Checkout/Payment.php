...
namespace Customweb\SagePayCw\Block\Adminhtml\Checkout;
...
class Payment extends \Magento\Backend\Block\Widget\Form\Container
{
	 ...
	 * @var string
	 */
	protected $_blockGroup = 'Customweb_SagePayCw';
	...
	protected function _construct()
	{
		...
		$this->setId('sagepaycw_checkout_payment');

		$this->buttonList->update('save', 'label', __('Submit Order'));
		$this->buttonList->update('save', 'class', 'primary');
		$this->buttonList->update('save', 'id', 'sagepaycw-payment-submit-button');
		...
		$this->buttonList->add(
			...
			[
				'label' => __('Cancel'),
				'class' => 'cancel',
				'id' => 'sagepaycw-payment-cancel-button'
			]
		);
	}
	...
	public function getTransaction()
	{
		if (!($this->transaction instanceof \Customweb\SagePayCw\Model\Authorization\Transaction)) {
			...
			if (!($transaction instanceof \Customweb\SagePayCw\Model\Authorization\Transaction) || !$transaction->getId()) {

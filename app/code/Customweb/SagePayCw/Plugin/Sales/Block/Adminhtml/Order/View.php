...
namespace Customweb\SagePayCw\Plugin\Sales\Block\Adminhtml\Order;
...
class View
{
	...
	public function aroundGetReviewPaymentUrl(\Magento\Sales\Block\Adminhtml\Order\View $subject, \Closure $proceed, $action)
    {
    	if ($subject->getOrder() instanceof \Magento\Sales\Model\Order
	    		...
	    		&& $subject->getOrder()->getPayment()->getMethodInstance() instanceof \Customweb\SagePayCw\Model\Payment\Method\AbstractMethod) {
		    ...
		    return $subject->getUrl('sagepaycw/transaction/reviewPayment', ['action' => $action, 'transaction_id' => $transaction->getId()]);
    	} else {
    ...
    {
		$this->_coreRegistry->register('sagepaycw_order_view', true, true);
    }
}
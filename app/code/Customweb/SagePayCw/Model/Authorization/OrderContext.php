...
class OrderContext implements \Customweb_Payment_Authorization_IOrderContext
{
	 ...
	 * @param \Customweb\SagePayCw\Model\Payment\Method\AbstractMethod|string $paymentMethod
	 * @throws \Exception
	 */
	...
	{
		return $this->_paymentHelper->getMethodInstance($this->paymentMethod);
	}
	...
	private function assembleDataFromQuote(
		...
		$checkoutId = $checkoutSession->getSagePayCwCheckoutId();
		if(empty($checkoutId)){
			$checkoutId = \Customweb_Core_Util_Rand::getUuid();
			$checkoutSession->setSagePayCwCheckoutId($checkoutId);
		}

...
abstract class Customweb_SagePay_Authorization_AbstractAdapter extends Customweb_SagePay_AbstractAdapter {
	 ...
	 */
	protected function processResponse(Customweb_SagePay_Authorization_Transaction $transaction){
		...
		if ($status == 'OK' || $status == 'AUTHENTICATED' || $status == 'REGISTERED' || $status == 'PAYPALOK ') {
			...
			$paymentMethod = Customweb_SagePay_Method_Factory::getMethod($transaction->getTransactionContext()->getOrderContext()->getPaymentMethod(),
			...
			// Check CV2
			if (isset($parameters['CV2Result']) &&
					 $transaction->getPaymentMethod()->existsPaymentMethodConfigurationValue("cv2_check_behavior") &&
					 $transaction->getAuthorizationMethod() != Customweb_Payment_Authorization_Recurring_IAdapter::AUTHORIZATION_METHOD_NAME &&
					 !$transaction->isMoto()) {
			...
			if (isset($parameters['FraudResponse']) &&
					 $transaction->getPaymentMethod()->existsPaymentMethodConfigurationValue("fraud_behavior")) {

...
$('.price, tr.price-row td, strong#unit-price, td.tar.pr, strong#incamt').each(function() {
    var $this = $(this);
    var text = $this.text();
...
$('#submits').prepend ("<p class='delvat'>Spend over £100 for FREE delivery<br>Price exclusive of VAT</p>");
$("select#pay-type option[value='PayPalSubscription']").remove();
$('th#d').append ("<span class='delcharge'>UK mainland: <b>Spend over £100 for free delivery</b> / Under £100 delivery is £10.50<br>Republic of Ireland, Northern Ireland, Scillies, Isle of Man, Scottish Islands and other countries: Please call us on 02920 708702 for a delivery cost or process your order and we will contact you.</span>");
...
$('<span class="ordermsg">For orders over 500 please call us on 029 2070 8702</span>').insertAfter($('body.personalised-lanyards-polyester #product input.qty'));
$('body.personalised-lanyards-polyester input.qty').on('keydown keyup', function(e){
    if ($(this).val() > 500 
        && e.keyCode != 46 // delete
...
/*CHECKOUT STUFF*/
$('<em>*</em>').insertBefore ($('input#del-address3'));
checkoutForm = $("body#www-idcardsandaccessories-co-uk.page.checkout #primary #checkout form#info");
if (checkoutForm.length){
      checkoutForm.submit(function(){ 
	      if ($.trim($("#primary fieldset.delivery-address input#del-address3").val()) == '' ){
              alert('Please enter your Delivery Town/City');
              return false;
          }
      });
}
$('.primary #checkout form li input').keyup(function(event) {
...
$('map').imageMapResize();
$('#feefologohere').insertBefore ($('#topphone'));
$('#product h1').append ($('#feefologohere'));
$('#lanyardsinfo').insertAfter('.product-list');
});